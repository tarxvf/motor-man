...
// 2. Hardware Interface: The Curtis 1205M typically uses INVERTED TTL serial signals.
//    You WILL need a suitable RS-232 to TTL converter that also INVERTS the signal,
//    or a custom circuit with an opto-isolator/transistor inverter, to interface with the
//    ESP32's standard TTL UART. Direct connection WILL NOT work due to voltage level
//    differences and signal inversion.
//    Connect the converter's TX to ESP32's RX, and converter's RX to ESP32's TX,
//    and ensure common GND.
// 3. ESP-NOW Peer MAC Address: Ensure 'peerMacAddress' is set to the MAC address
//    of the ESP-NOW device you want to communicate with. For broadcast, use all 0xFF.
// 4. Data Interpretation: The raw hex values read from Curtis need to be correctly
