...
If you DON'T want the science experiment, and JUST want a touch screen display with serial chip and 10 feet of cable to connect to your existing Curtis controller and get all the cool features (including the digital rain lock screen) .. then hit up the [Little Blue Art Car Donation Center](https://littlebleu.org) and we'll ship you a kit in return for a donation to the Art Car fund. Your Gift makes it possible to take this car out and inspire builders everywhere to make something artistic no matter what the build medium. 

## Project Overview
(yes.. cheaper than Alltrax) - I think alltrax is a premium well built product, but unfortunately everything we do is on a budget, and I can buy several 500 Amp Curtis controllers for the price of one 500 Amp Alltrax.. (if somebody over at Alltrax wants to send .. a gently used 500 Amp controller.. that would facilitate updating this project to control alltrax controllers..) and I wouldn't mind putting one in the Taylor Dunn for Road Testing.. 

Additional Features: If you have a 10 foot long 1968 Taylor-Dunn B2-48 like we have, then you'll appreciate the ESP-NOW features allowing you to have one Display Controller on the Motor and one Display Controller up on the driver dash without any cables in between for all your controls and vital displays (except the throttle.) - I haven't myself decided to bite off on the drive by wire over the ESP-NOW network... yet..
...
- Control an Alltrax controller (shameless plug here for a unit to test this project with.) 
...
###Hardware
1. A Cheap Yellow Display (CYD). Many on the market. Typically its actually called an "ESP32-2432SO28R" so you can see why .. it gets shortened to CYD.  I usually get 2 for half the price of a steak at Ruth Chris on that Jungle store [->here<-](https://amazon.com)
